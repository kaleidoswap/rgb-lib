...
fn execute_swap(
    ...
    let completion = maker
        ...
        .unwrap();
    assert_consignment_transport(&completion.consignments, proxy_url);
    ...
    let txn = maker.database().begin_transaction().unwrap();
    maker.broadcast_psbt(&txn, &finalized_psbt).unwrap();
    txn.commit().unwrap();
    mine(false);
...
fn assert_consignment_transport(consignments: &[OnchainSwapConsignment], proxy_url: Option<&str>) {
    ...
    for consignment in consignments {
        match proxy_url {
            ...
            None => assert!(consignment.endpoint.is_none()),
        }
    }
}
